...
export class AiService {
  ...
  async chat(prompt: string, sessionId = 'default') {
    ...
    const response = await this.requestCompletions(config, messages, false);
  ...
  async chatStream(
    ...
    const response = await this.requestCompletions(config, messages, true);
  ...
  private async requestCompletions(
    ...
    const urls = this.buildCandidateBaseUrls(config.baseUrl);
    let lastErrorText = '';
    ...
    for (const baseUrl of urls) {
      ...
      const shouldRetry =
        (response.status === 404 || /Not\s*support/i.test(errorText)) &&
        baseUrl.includes('/apps/protocols/compatible-mode/v1');

      if (!shouldRetry) {
        const message = stream ? '千问流式调用失败' : '千问模型调用失败';
  ...
  private buildCandidateBaseUrls(baseUrl: string) {
    const normalized = baseUrl.replace(/\/+$/, '');
    const fallback = 'https://dashscope.aliyuncs.com/compatible-mode/v1';

    if (normalized === fallback) {
      return [normalized];
    }

    if (normalized.includes('/apps/protocols/compatible-mode/v1')) {
  ...
  private getModelConfig(): QwenConfig {
